...
class DirectAUTuner:
    ...
    def run(self):
        ...
        print(f"\nDirectAU Hyperparameter Tuning - Total combinations: {len(combinations)}\n{'='*80}")
        for i, combo in enumerate(combinations, 1):
            ...
            try:
                ...
                print(f"Reg lambda: {conf['reg.lambda']}, Optimizer: {conf['optimizer']}")
                print(f"Gamma: {conf['DirectAU']['gamma']}, n_layers: {conf['DirectAU']['n_layers']}")
    ...
    def save(self, path="directau_results.json"):
...
if __name__ == '__main__':
    ...
    print("- Embedding size: [16, 32, 64, 128, 256, 512]")
    print("- Batch size: [16, 32, 64, 128, 256, 512, 1024, 2048]")
    print("- Learning rate: [1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 5e-3]")
    print("- Regularization lambda: [1e-5, 1e-4, 1e-3]")
    print("- Optimizer: ['adam', 'sgd']")
    print("- Gamma: [0.5, 1.0, 3.0]")
    print("- n_layers: [1, 2, 3, 4, 5, 6]")
    # print(f"Total combinations:  31104 = {5*8*6*6*3*2*3*3}\n")

...
class SelfCFTuner:
    def __init__(self, training_set, test_set, results_dir="./tuning_results/"):
    ...
    def generate_configs(self):
        ...
        # Extract SelfCF specific parameters
        selfcf_params = self.hyperparameters['SelfCF']
        base_params = {k: v for k, v in self.hyperparameters.items() if k != 'SelfCF'}
        
        # Generate all combinations
        base_keys = list(base_params.keys())
        base_values = list(base_params.values())
        
        selfcf_keys = list(selfcf_params.keys())
        selfcf_values = list(selfcf_params.values())
        
        configs = []
        
        for base_combo in itertools.product(*base_values):
            for selfcf_combo in itertools.product(*selfcf_values):
                config = dict(zip(base_keys, base_combo))
                config['SelfCF'] = dict(zip(selfcf_keys, selfcf_combo))
                config['model'] = {'name': 'SelfCF', 'type': 'graph'}
                config['item.ranking.topN'] = [10, 20, 30, 50]
                config['output'] = self.results_dir
    ...
    def run_single_experiment(self, config, experiment_id):
        ...
        try:
            ...
            model_conf = ModelConf(config_dict=config)
            
            # Initialize and train model
            model = SelfCF(model_conf, self.training_set, self.test_set)
            model.execute()
            
            # Extract performance metrics
            if model.bestPerformance:
                performance = model.bestPerformance[1]
                best_epoch = model.bestPerformance[0]
            else:
                performance = {}
                best_epoch = 0
            ...
            if performance and (self.best_performance is None or 
                              performance.get('Recall', 0) > self.best_performance.get('Recall', 0)):
                self.best_performance = performance
                self.best_config = config
                print(f"New best performance found: Recall = {performance.get('Recall', 0):.4f}")
    ...
    def run_tuning(self):
        ...
        print(f"Starting hyperparameter tuning with {total_experiments} configurations")
...
if __name__ == '__main__':
    ...
    tuner = SelfCFTuner(train_data, test_data)
    tuner.run_tuning()